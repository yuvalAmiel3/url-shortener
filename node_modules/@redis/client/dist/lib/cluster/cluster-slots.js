 ...
 * `smigratedSeqIdsSeen`).
 */
const DEFAULT_CURSOR_MAX_IDLE_MS = 300000;
class RedisClusterSlots {
    ...
    constructor(options, emit, clusterClientId) {
        ...
        this.#himportRegistry = options.himportRegistry ?? new registry_1.FieldsetRegistry();
            ...
            try {
                ...
                for (const { addr: { host, port }, slots } of entry.destinations) {
                    ...
                    if (destMasterNode.client) {
                        ...
                        sourceNode.client._getQueue().rejectCommands(commandsForDestination, new errors_1.DisconnectsClientError());
                        (0, enterprise_maintenance_manager_1.dbgMaintenance)(`[CSlots]: Rejected ${commandsForDestination.length} commands for ${destinationSlots.size} slots because ${destMasterNode.address} has no client`);
                    }
                ...
                if (sourceStillHasSlots) {
                    ...
                    if (remainingCommands.length > 0) {
                        if (lastDestNode?.client) {
                            ...
                            sourceNode.client._getQueue().rejectCommands(remainingCommands, new errors_1.DisconnectsClientError());
                            (0, enterprise_maintenance_manager_1.dbgMaintenance)(`[CSlots]: Rejected ${remainingCommands.length} remaining commands because no destination client was available`);
                        }
                    }
                    if ('pubSub' in sourceNode) {
    ...
    *#clients() {
    ...
    #assertReady() {
        ...
        if (!this.#isReady) {
            throw new errors_1.ClientOfflineError();
        }
    }
    /**
    ...
    getClientForKey(key, isReadonly) {
        this.#assertReady();
        const slotNumber = (0, cluster_key_slot_1.default)(key);
        if (isReadonly) {
