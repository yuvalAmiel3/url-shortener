...
const dispatch_1 = require("./request-response-policies/dispatch");
const cluster_key_slot_1 = __importDefault(require("cluster-key-slot"));
const ft_cursor_1 = require("./request-response-policies/ft-cursor");
const scan_cursor_1 = require("./request-response-policies/scan-cursor");
class RedisCluster extends node_events_1.EventEmitter {
    static #createCommand(command, resp) {
        ...
        return async function (...args) {
            const parser = new parser_1.BasicCommandParser(this._self._keyPrefix);
            command.parseCommand(parser, ...args);
            return this._self._executeWithPolicies(parser, command.IS_READ_ONLY, this._commandOptions, p => (client, opts) => client._executeCommand(command, p, opts, transformReply));
        };
    }
    static #createModuleCommand(command, resp) {
        ...
        return async function (...args) {
            const parser = new parser_1.BasicCommandParser(this._self._keyPrefix);
            command.parseCommand(parser, ...args);
            return this._self._executeWithPolicies(parser, command.IS_READ_ONLY, this._self._commandOptions, p => (client, opts) => client._executeCommand(command, p, opts, transformReply));
        };
    }
    static #createFunctionCommand(name, fn, resp) {
        ...
        return async function (...args) {
            ...
            return this._self._executeWithPolicies(parser, fn.IS_READ_ONLY, this._self._commandOptions, p => (client, opts) => client._executeCommand(fn, p, opts, transformReply));
        };
    }
    static #createScriptCommand(script, resp) {
        ...
        return async function (...args) {
            ...
            return this._self._executeWithPolicies(parser, script.IS_READ_ONLY, this._commandOptions, p => (client, opts) => client._executeScript(script, p, opts, transformReply));
        };
    }
    ...
    constructor(options) {
        ...
        this._policyResolver = command_metadata_1.defaultCommandMetadata;
    }
    duplicate(overrides) {
     ...
     */
    async _executeWithPolicies(parser, isReadonly, options, makeFn) {
        ...
        const policy = policyResult.ok
            ? policyResult.value
            : (0, command_metadata_1.defaultCommandPolicies)(parser.keys.length === 0);
        ...
        // (see `isReplicaSafe`).
        const readonly = (0, command_metadata_1.isReplicaSafe)(policy, isReadonly);
        ...
        if (requestPolicy === command_metadata_1.REQUEST_POLICIES_WITH_DEFAULTS.DEFAULT_KEYED &&
            responsePolicy === command_metadata_1.RESPONSE_POLICIES_WITH_DEFAULTS.DEFAULT_KEYED) {
            return this._execute(parser, readonly, options, makeFn(parser));
        }
        // https://redis.io/docs/latest/develop/reference/command-tips
        const router = dispatch_1.REQUEST_ROUTERS[requestPolicy];
        if (!router) {
        ...
        const reducer = dispatch_1.RESPONSE_REDUCERS[responsePolicy];
        if (!reducer) {
        ...
        const plan = await router(this._slots, parser, readonly, policy.keySpecs);
        if (plan.length === 0) {
        ...
        const numericAgg = dispatch_1.NUMERIC_AGG_POLICIES.has(responsePolicy);
        ...
        const responsePromises = plan.map(async (entry) => {
            ...
            const client = (entry.client ?? (entry.getClient ? await entry.getClient() : undefined));
            return this._execute(entryParser, readonly, execOptions, makeFn(entryParser), client, plan.length === 1 ? served : undefined);
        });
        const positionHints = plan.map(entry => entry.groupIndices);
        let reply = await reducer(responsePromises, parser, positionHints);
        if (numericAgg) {
            reply = (0, dispatch_1.remapAggregateReply)(reply, requestedMapping);
        }
        ...
        const servedClient = served.client;
        ...
        try {
            reply = (0, ft_cursor_1.finalizeFtCursor)(this._slots, parser, hookPlan, reply);
        }
        ...
        try {
            reply = (0, scan_cursor_1.finalizeScanCursor)(this._slots, parser, hookPlan, reply);
        }
     ...
     */
    async _execute(parser, isReadonly, options, fn, pinnedClient, 
    ...
    served) {
        ...
        if (pinnedClient) {
            client = pinnedClient;
            slotNumber = parser.firstKey === undefined ? undefined : (0, cluster_key_slot_1.default)(parser.firstKey);
        }
        else {
    ...
    async sendCommand(firstKey, isReadonly, args, options) {
        ...
        if (firstKey !== undefined)
            parser.markRoutingKey(firstKey);
        ...
        return this._self._executeWithPolicies(parser, isReadonly, opts, p => (client, opts) => client.sendCommand(p.redisArgs, opts));
    }
    MULTI(routing) {
        ...
        }, routing, this._commandOptions?.typeMapping, this._self._keyPrefix);
    }
    multi = this.MULTI;
     ...
     */
    getNodeClientForKey(key, isReadonly) {
