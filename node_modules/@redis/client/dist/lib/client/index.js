...
const registry_1 = require("../himport/registry");
const HIMPORT_DISCARD_1 = __importDefault(require("../commands/HIMPORT_DISCARD"));
const HIMPORT_DISCARDALL_1 = __importDefault(require("../commands/HIMPORT_DISCARDALL"));
const HIMPORT_PREPARE_1 = __importDefault(require("../commands/HIMPORT_PREPARE"));
const HIMPORT_SET_1 = __importDefault(require("../commands/HIMPORT_SET"));
 ...
 */
function assertNoHimportSessionCommands(commands) {
    for (const { args } of commands) {
        if (String(args[0]).toUpperCase() !== 'HIMPORT')
            continue;
        if (HIMPORT_SESSION_SUBCOMMANDS.has(String(args[1]).toUpperCase())) {
            throw new Error('HIMPORT PREPARE/DISCARD/DISCARDALL are not supported inside MULTI/pipeline; call them on the client before the transaction');
        }
    }
}
;
 ...
 * replay path instead.
 */
function isPreEnqueueError(err) {
    return err instanceof errors_1.ClientClosedError ||
        err instanceof errors_1.ClientOfflineError ||
        err instanceof errors_1.AbortError ||
...
class RedisClient extends node_events_1.EventEmitter {
    ...
    #himportRegistry;
    #preparedFieldsets = new registry_1.PreparedFieldsets();
    ...
    constructor(options) {
        ...
        this.#himportRegistry = this.#options.himportRegistry ?? new registry_1.FieldsetRegistry();
    ...
    async _executeCommand(command, parser, commandOptions, transformReply) {
        if (command === HIMPORT_SET_1.default || command === HIMPORT_PREPARE_1.default ||
            command === HIMPORT_DISCARD_1.default || command === HIMPORT_DISCARDALL_1.default) {
        ...
        if (csc) {
            ...
            const cacheable = (0, command_metadata_1.isCacheable)(command_metadata_1.defaultCommandMetadata.lookup(parser.commandIdentifier), command.CACHEABLE);
    ...
    client, command, parser, commandOptions, transformReply, retried = false) {
        ...
        const injectOpts = { asap: effectiveAsap, chainId: effectiveChainId };
        ...
        if (prepared.syncedDiscardCount < registry.discardCount) {
            ...
            const rollbackVersions = new Map();
        ...
        let setName;
        let userDiscard;
        if (command === HIMPORT_PREPARE_1.default) {
        ...
        else if (command === HIMPORT_SET_1.default) {
            ...
            if (entry !== undefined) {
                ...
                if (sessionVersion === undefined || sessionVersion < entry.version) {
                    ...
                    prelude.push({
                        ...
                        rollback: () => {
                            if (prepared.get(name) === entry.version)
                                prepared.delete(name);
                        },
        ...
        else if (command === HIMPORT_DISCARD_1.default) {
            const name = String(args[2]);
            const sessionVersion = prepared.get(name);
        ...
        if (command === HIMPORT_SET_1.default && commandOptions?.askRedirect && prelude.length > 0) {
            prelude.push({ args: [ASKING_1.ASKING_CMD], rollback: () => { } });
        }
        ...
        try {
            ...
            if (command === HIMPORT_PREPARE_1.default && userPrepare !== undefined) {
            ...
            else if (command === HIMPORT_SET_1.default && setName !== undefined && !retried &&
                err?.message?.includes?.('no such fieldset') &&
                ...
                if (isPreEnqueueError(err)) {
                    for (const [name, fields] of userDiscard.removed) {
                        if (registry.get(name) === undefined)
                            registry.set(name, fields);
                    }
                }
                for (const [name, version] of userDiscard.restore) {
                    if (prepared.get(name) === undefined)
                        prepared.set(name, version);
                }
        ...
        (0, tracing_1.publish)(tracing_1.CHANNELS.COMMAND_REPLY, () => ({ args: (0, tracing_1.sanitizeArgs)(parser.redisArgs), reply: finalReply, clientId: this._clientId }));
        return finalReply;
    }
    /**
     * @internal
    ...
    async WATCH(key) {
        ...
        // optimistic lock.
        const reply = await this._self.sendCommand(['WATCH', ...(0, parser_1.prefixKeys)(this._self._keyPrefix, key)]);
    ...
    MULTI() {
        return new (this.Multi)(this._executeMulti.bind(this), this._executePipeline.bind(this), this._commandOptions?.typeMapping, this._self._keyPrefix);
    }
    multi = this.MULTI;
    /**
