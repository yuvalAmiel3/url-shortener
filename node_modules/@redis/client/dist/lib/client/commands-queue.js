...
class RedisCommandsQueue {
    ...
    setMaintenanceCommandTimeout(ms) {
        ...
        for (const node of this.#toWrite.nodes()) {
            ...
            _a.#removeTimeoutListener(command);
            counter++;
    ...
    #createAbortListener(node, value) {
        return () => {
            this.#rejectCommand(node, value, new errors_1.AbortError());
        };
    }
    #createTimeoutListener(node, value, effectiveTimeout, wasInMaintenance) {
        return () => {
            this.#rejectCommand(node, value, wasInMaintenance
                ? new errors_1.CommandTimeoutDuringMaintenanceError(effectiveTimeout)
                : new errors_1.TimeoutError());
        };
    }
    #rejectCommand(node, value, err) {
        if (value.rejected)
            return;
        value.rejected = true;
        _a.#removeCommandListeners(value);
    ...
    *commandsToWrite() {
        ...
        while (toSend) {
            ...
            if (toSend.abort) {
                _a.#removeAbortListener(toSend);
                toSend.abort = undefined;
            }
            if (toSend.timeout) {
                _a.#removeTimeoutListener(toSend);
    ...
    static #removeCommandListeners(command) {
        if (command.abort) {
            _a.#removeAbortListener(command);
    ...
    static #flushToWrite(toBeSent, err) {
        if (toBeSent.abort) {
            _a.#removeAbortListener(toBeSent);
        }
        if (toBeSent.timeout) {
            _a.#removeTimeoutListener(toBeSent);
        }
        toBeSent.reject(err);
    }
    flushWaitingForReply(err) {
        ...
        while (this.#toWrite.head?.value.chainId === this.#chainInExecution) {
            _a.#flushToWrite(this.#toWrite.shift(), err);
        }
     ...
     */
    rejectCommands(commands, err) {
        for (const command of commands) {
            if (command.rejected)
                continue;
            command.rejected = true;
            _a.#removeCommandListeners(command);
            command.reject(err);
        }
    }
    /**
    ...
    prependCommandsToWrite(commands) {
        ...
        for (let i = commands.length - 1; i >= 0; i--) {
            ...
            if (value.timeout) {
                _a.#removeTimeoutListener(value);
                if (value.timeout.signal.aborted) {
                    this.#rejectCommand(node, value, value.timeout.wasInMaintenance
                        ? new errors_1.CommandTimeoutDuringMaintenanceError(value.timeout.effectiveTimeout)
                        : new errors_1.TimeoutError());
                    continue;
                }
                else {
                    value.timeout.listener = this.#createTimeoutListener(node, value, value.timeout.effectiveTimeout, value.timeout.wasInMaintenance);
